...
def estimation_1():
    global graph, heighest_weight, nearer
    while ((positions[heighest_weight[1]][0] - positions[heighest_weight[2]][0]) ** 2 + (positions[heighest_weight[1]][1] - positions[heighest_weight[2]][1]) ** 2)/nearer > heighest_weight[0]:
        nearer *= 2
...
def visiting(node):
    global positions, shortest_distance, estimation, visited, queue, graph
    for i in graph[node].keys():
        distance_from_node = shortest_distance[node][0] + graph[node][i]
        print(distance_from_node, shortest_distance[i], i)
...
def a_star(node_1, node_2, window):
    global graph, positions, shortest_distance, estimation, queue, visited, nearer
    starting = node_1.get()
    ending = node_2.get()
    window.destroy()
    estimation_1()
    ...
    for i in graph.keys():
        estimation[i] = round(
            (positions[i][0] - positions[ending][0]) ** 2 + (positions[i][1] - positions[ending][1]) ** 2)//nearer
        shortest_distance[i] = [99999999, i]
    shortest_distance[starting] = [0, starting]
    while queue:
        temp = [99999999999, 1]
        for i in queue:
            if shortest_distance[i][0] + estimation[i] < temp[0]:
...
def starting_ending(window_1=None):
    ...
    z = Button(window, text="Enter",
               command=lambda node_1=node_1, node_2=node_2, window=window: a_star(node_1, node_2, window))
    z.grid(row=2, column=1)
    window.mainloop()


heighest_weight = [0, 0, 0]
nearer = 1


def from_click_third(node, click_2, window):
    global graph, click_1, heighest_weight
    weight = int(node.get())
    window.destroy()
    ...
    write(weight)
    if heighest_weight[0] < weight:
        heighest_weight = [weight, click_1, click_2]
